...
parser.add_argument('--pitch_path',
                    type=str,
                    required=True,
...
def note_search(data, file_path):
    onset_prob, pitch_prob = np.split(data, [
        1,
    ], axis=-1)
    ...
    for idx in valid_indexs[1:]:
        if idx - last > 1:
            onset_max_idx = np.argmax(onset_prob[start:last + 1, 0],
                                      axis=0) + start
            pitch_max_idxs = np.argmax(pitch_prob[start:last + 1, :],
                                       axis=0) + start
            ...
            for i in range(88):
                ...
                if pitch_prob[pitch_max_idx, i] > pitch_thrd:
                    buffer_notes.append([
                        (pitch_max_idx + onset_max_idx) / 2 * hop_len / sr,
                        i + 21
                    ])
...
def raw_res(args):
    ...
    filenames = [
        os.path.join(args.raw_pitch_path, filename)
        for filename in os.listdir(args.raw_pitch_path) if filename.replace(
            '.pitch_raw', '.res') not in os.listdir(args.result_path)
    ]
    for filename in filenames:
        basename = os.path.basename(filename)
        print('............... predict %s ...............\n' % basename)
        pitch_data = np.loadtxt(filename)
        onset_data = np.loadtxt(
            os.path.join(args.raw_onset_path,
                         basename.replace('.pitch_raw', '.onset_raw')))
        ...
        file_path = os.path.join(result_dir,
                                 basename.replace('.pitch_raw', '.res'))
